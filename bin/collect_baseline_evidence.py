...
def specs():
    ...
    cloud = "<SCPLAD_DATA_ROOT>/Squidiff_cloud_20260307"
    for name, model in [
        ...
        add("<PRIVATE_HOST>", cloud + "/scripts_tmp/" + name, f"baselines/experiment_runners/{model}/{name}")
    for name in ["metadata.json", "summary.json", "model/config.pkl"]:
        add("<PRIVATE_HOST>", cloud + "/datasets/otherdata/txpert/txpert_k562_pathway5000_module/gears_fixed2000_v1/" + name,
            "configs/baselines/GEARS/k562_only/" + Path(name).name)
    add("<PRIVATE_HOST>", cloud + "/third_party/scouter_runs/txpert_pathway5000_unseen_geneptv1_official40_npred2000_v1/run_summary.json",
    ...
    for name in ["run_cellflow_1000k_generate2000_20260511.sh",
        ...
        add("<PRIVATE_HOST>", "<SCPLAD_DATA_ROOT>/CellFlow_runs/" + name,
            "baselines/experiment_runners/CellFlow/" + name)
    add("<PRIVATE_HOST>", "<SCPLAD_DATA_ROOT>/CellFlow_runs/txpert_pathway5000_esm2_t36_3B_1000k_v1/smoke_meta.json",
    ...
    for name in ["txpert_pathway3352_xcell_zeroshot_k562.toml", "run_state_bs8_30k_training.sh",
        ...
        add("<PRIVATE_HOST>", "<SCPLAD_DATA_ROOT>/third_party/state_py39/benchmarks/" + name,
            "baselines/experiment_runners/STATE/" + name)
    transport = "<SCPLAD_DATA_ROOT>/Squidiff_transport_20260601"
    for name in ["generate_txpert_scplad_predictions_fixed.py", "eval_txpert_scplad_fixed2000_pra_corr.py"]:
        add("<PRIVATE_HOST>", transport + "/" + name, "baselines/experiment_runners/TxPert/" + name)
    add("<PRIVATE_HOST>", transport + "/scripts_tmp/current_txpert_official_fixed2000/run_txpert_official_k562_all_fixed2000_sharded4.sh",
        "baselines/experiment_runners/TxPert/run_txpert_official_k562_all_fixed2000_sharded4.sh")
    add("<PRIVATE_HOST>", transport + "/scripts_tmp/generate_source_expression_baselines.py",
