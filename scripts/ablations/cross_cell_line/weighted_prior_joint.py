...
def read_condition_set(path):
    path = Path(path)
    if not path.is_file():
        raise FileNotFoundError(f"heldout_condition_file not found: {path}")
    conditions = {
        line.strip()
    ...
    if not conditions:
        raise ValueError(f"heldout_condition_file is empty: {path}")
    return conditions
...
def enforce_diffusion_boundary(train_obs, control_obs, args):
    ...
    if len(leaked) and not args.allow_heldout_context_train:
        raise ValueError(
    ...
    if args.allow_heldout_context_train:
        if not args.heldout_condition_file:
            raise ValueError(
                "--allow_heldout_context_train requires --heldout_condition_file so that "
                "target-context seen conditions cannot overlap the evaluation conditions."
            )
        if args.condition_key not in train_obs.columns:
            raise KeyError(f"condition_key={args.condition_key} not found in train_h5ad obs")
        heldout_conditions = read_condition_set(args.heldout_condition_file)
        target_context_conditions = set(
            train_obs.loc[train_context == heldout, args.condition_key].astype(str)
        )
        overlap = sorted(target_context_conditions & heldout_conditions)
        if overlap:
            preview = ", ".join(overlap[:10])
            raise ValueError(
                f"Diffusion train_h5ad contains {len(overlap)} held-out perturbation conditions: "
                f"{preview}"
            )
        if not len(leaked):
            raise ValueError(
                f"--allow_heldout_context_train was set, but train_h5ad has no {heldout} rows."
            )
...
def main():
    ...
    parser.add_argument(
        "--allow_heldout_context_train",
        action="store_true",
        help=(
            "Allow seen perturbations from heldout_context in diffusion training. "
            "Requires --heldout_condition_file and rejects any condition overlap."
        ),
    )
    parser.add_argument(
        "--heldout_condition_file",
        default="",
        help="Text file containing one held-out perturbation condition per line.",
    )
    parser.add_argument(
        "--control_labels",
    ...
    config.update(
        {
            ...
            "data_boundary": {
                ...
                "heldout_context_absent_from_diffusion_train": not bool(
                    args.allow_heldout_context_train
                ),
                "heldout_context_seen_conditions_allowed": bool(
                    args.allow_heldout_context_train
                ),
                "heldout_condition_file": str(args.heldout_condition_file),
                "heldout_conditions_absent_from_heldout_context_train": True,
