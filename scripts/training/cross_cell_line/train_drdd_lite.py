...
class WeightedBlockAdaLNConditionDenoiser(GOControlPriorFeatureDenoiser):
    """
    AdaLN DRDD-lite denoiser with block-wise biological prior fusion.

    This keeps the main prior_feature_cond conditioning path: prior/control
    tokens modulate AdaLN Transformer blocks. The only change is replacing the
    flat condition encoder with per-prior-source projectors and learned softmax
    fusion weights.
    """
...
def init_ema_state(model):
    return {key: value.detach().clone() for key, value in raw_state_dict(model).items()}


@torch.no_grad()
def update_ema_state(ema_state, model, decay):
    current_state = raw_state_dict(model)
    for key, value in current_state.items():
        value = value.detach()
        if torch.is_floating_point(value):
            ema_state[key].mul_(float(decay)).add_(value, alpha=1.0 - float(decay))
        else:
            ema_state[key].copy_(value)


def save_ema_checkpoint(path, ema_state, step, config, decay):
    payload = {
        "model": {key: value.detach().cpu() for key, value in ema_state.items()},
        "optimizer": None,
        "scaler": None,
        "step": int(step),
        "config": {**config, "ema_checkpoint": True, "ema_decay": float(decay)},
...
def main():
    ...
    parser.add_argument(
        ...
        choices=[
            ...
            "prior_feature_cond",
            "direct_add_cond",
            "weighted_prior_direct_add",
            "weighted_prior_adaln",
        ],
        default="adaln",
        help=(
            ...
            "and injects it by direct addition; weighted_prior_adaln uses the same block-wise "
            "fusion but keeps AdaLN modulation."
        ),
    )
    parser.add_argument(
        "--target_space",
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.0,
        help="If >0, maintain an exponential moving average of model weights for evaluation checkpoints.",
    )
    parser.add_argument(
        "--ema_save_interval",
        type=int,
        default=0,
        help="Save EMA checkpoints every N steps; 0 means use save_interval.",
    )
    ...
    elif str(args.model_variant) == "weighted_prior_adaln":
        model_cls = WeightedBlockAdaLNConditionDenoiser
    else:
    ...
    if str(args.model_variant) in {"weighted_prior_direct_add", "weighted_prior_adaln"}:
    ...
    ema_save_interval = int(args.ema_save_interval or args.save_interval)
    if ema_decay > 0:
        ema_state = resume_ema_state if resume_ema_state is not None else init_ema_state(model)
    epoch = 0
    ...
    for step in range(start_step + 1, args.steps + 1):
        ...
        if step % args.log_interval == 0 or step == 1:
            ...
            if is_main(rank):
                ...
                if ema_state is not None:
                    row["ema_weight_decay"] = float(ema_decay)
        ...
        if (
            ...
            and (step % ema_save_interval == 0 or step == args.steps)
        ):
            save_ema_checkpoint(
                output_dir / f"model_ema_step_{step}.pt",
                ema_state,
                step,
                config,
                ema_decay,
    ...
    if is_main(rank):
        ...
        if ema_state is not None:
            save_ema_checkpoint(output_dir / "model_ema_final.pt", ema_state, args.steps, config, ema_decay)
