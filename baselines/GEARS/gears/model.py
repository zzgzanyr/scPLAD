        ...
        if self.no_perturb:
            ...
            for idx, i in enumerate(pert_idx):
                if torch.is_tensor(i):
                    if i.dim() == 0:
                        iter_vals = [int(i.item())]
                    else:
                        iter_vals = [int(v) for v in i.view(-1).tolist()]
                elif isinstance(i, (list, tuple)):
                    iter_vals = [int(v) for v in i]
                else:
                    iter_vals = [int(i)]
            ...
            if len(pert_index) == 0:
                ...
                pert_index = torch.tensor(
                    pert_index, dtype=torch.long, device=self.args['device']
                ).T
