...
class PertDataCellGraph(Data):
    """PyG Data object that keeps perturbation ids fixed across mini-batch collation."""
...
# Rebind original PertData methods after introducing the custom PyG Data subclass.
# This keeps the file change minimal while ensuring PertData still exposes the
# full dataset-processing API expected by the rest of GEARS.
for _method_name in [
    "set_pert_genes",
    "load",
    "new_data_process",
    ...
    setattr(PertData, _method_name, getattr(PertDataCellGraph, _method_name))
