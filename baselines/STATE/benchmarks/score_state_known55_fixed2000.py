...
def main() -> None:
    ...
    conditions = pd.read_csv(KNOWN55_CSV)["condition"].astype(str).tolist()

    pred = ad.read_h5ad(PRED_H5AD)
    pred_labels = pred.obs["condition"].astype(str).to_numpy()
    pred_x = dense_float32(pred.X)

    test_backed = ad.read_h5ad(TEST_H5AD, backed="r")
    ...
    control_backed = ad.read_h5ad(CONTROL_H5AD, backed="r")
    ...
    for condition in conditions:
        ...
        if pred_values.shape[0] != 2000:
            raise ValueError(f"{condition}: expected 2000 predictions, got {pred_values.shape[0]}")
    ...
    summary = {
        "model": "state_20k_known55_fixed2000",
    ...
    per_condition.to_csv(OUTPUT_DIR / "per_condition_metrics.csv", index=False)
